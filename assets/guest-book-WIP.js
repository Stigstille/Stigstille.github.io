    ...
    fetch(
        ...
        .then((data) => {
            ...
            data.forEach((row) => {
                ...
                // Sanitize Data
                let SantizeResponses = encodeHTML(row.Mod_Link)

                let SantizeName = encodeHTML(row.Username)
...
var swear_words_arr = new Array("IAMDISABLEINGTHESWEARWORDTHINGYLMAONOONEISGOINGTOGUESSTHISPLEASEDONTLOOKATSOURCECODE");
...
function validate_text() {
    reset_alert_count();
    var compare_text = document.getElementById(GOOGLE_ENTRY_ID_Link).value;
...
<h1  style="text-align: center;
    margin-top: 2em;">Your message will not be added! \nThe following illegal words were found:</h1> <p>${alert_text}. If you belive this is a mistake, please contact Stigstille#7507</p>`
        // Fade message in
...
<h1 style="text-align: center;
    margin-top: 2em;">Your Mod Has Added to the List! It should appear shortly!</h1> `
...
function ResetSwearForm() {
    ...
    subscribeForm.innerHTML = ` <h1>Upload your mod</h1>
    ...
    <form name="gform" id="gform" enctype="text/plain" action="${Google_Form_Link}" target="hidden_iframe">
       ...
       <label for="${GOOGLE_ENTRY_ID_Name}"  class="aterisk_after">Email </label>
        ...
        <label for="${GOOGLE_ENTRY_ID_Email}" class="aterisk_after">Username </label>
      
   
        <input class="form-element short" type="text" name="${GOOGLE_ENTRY_ID_Email}" id="${GOOGLE_ENTRY_ID_Email}" placeholder="MarketingPip" style="background: #5EC6C8;" required>
      
   <label for="${GOOGLE_ENTRY_ID_Guestbook}"  class="max-length">Link to your Mod </label>
